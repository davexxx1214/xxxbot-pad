...
class ToolExtensionMixin(WechatAPIClientBase):
            ...
            try:
                ...
                if json_resp.get("Success"):
                    ...
                    if self._is_ios_error(json_resp):
                        logger.warning(f"[API-Client3] 检测到iOS设备-104错误，启用iOS兼容模式")
                        
                        try:
                            # 尝试使用16KB小分段下载
                            ios_result = await self._try_ios_compatible_download(session, msg_id, to_wxid, data_len, start_pos)
                            if ios_result:
                                logger.info(f"[API-Client3] ✅ iOS兼容模式下载成功: {len(ios_result)} 字节")
                                return ios_result
                            else:
                                logger.warning(f"[API-Client3] ❌ iOS兼容模式下载也失败")
                        except Exception as ios_err:
                            logger.error(f"[API-Client3] iOS兼容模式执行失败: {ios_err}")
    ...
    def _is_ios_error(self, response_dict):
        ...
        try:
            ...
            logger.debug(f"[API-Client3] iOS错误检测异常: {e}")
            return False
    ...
    async def _try_ios_compatible_download(self, session, msg_id, to_wxid, data_len, start_pos):
        ...
        try:
            # 方法1: 使用16KB小分段
            logger.debug(f"[API-Client3] iOS模式方法1: 使用16KB小分段下载")
            ...
            logger.debug(f"[API-Client3] iOS模式参数: StartPos={start_pos}, ChunkSize={current_chunk_size}")
            ...
            # 方法2: 尝试其他iOS兼容的API调用
            logger.debug(f"[API-Client3] iOS模式方法2: 尝试其他API调用")
            ...
            logger.error(f"[API-Client3] iOS兼容性下载异常: {e}")
            return None
