...
class _mjApi:
    ...
    def help_text(self):
        ...
        help_text += f"(2) imagine垫图:发送['{self.pad_prefix[0]} + prompt描述'],然后发送一张图片进行生成\n"
        help_text += f"(3) 图片放大和变换:使用['{self.up_prefix[0]} + 任务ID操作']即可放大和变换imagine生成的图片\n"
        help_text += f"(4) describe识图:直接发送图片即可帮你识别解析prompt描述,或发送['{self.describe_prefix[0]}']+图片亦可\n"
        help_text += f"(5) blend混图:发送['{self.blend_prefix[0]}']指令，然后发送多张图片最后发送['{self.end_prefix[0]}']进行混合\n"
